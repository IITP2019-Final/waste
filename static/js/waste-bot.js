...
/*      $(document).ready( function (data) {
      ...
      $.post('/chatbots', {'text': res}).done(function(data){
        ...
        // data['result'] = 0 (비용), 1(업체), 2(방법)
          botui.message
            .bot({
              ...
              loading: true,
              type: 'html',
              content: '<div class="answer-table">폐기물 <u>처리비용</u> 에요!<br><br>' //+ res.value
              + '<table border=1><thead>' + '<tr>' + '<th>카테고리</th><th>품목</th><th>규격</th><th>부과금액(단위:원)</th>' + '</tr>' + '</thead>' + '<tbody>' + ws + '</tbody>' + '</table></div>'
              + '<div class="answer-table2">*대형폐가전(원형보전) 또는 재활용 가능 시 : <b>무상수거</b> 가능해요!<br>궁금한 사항이 더 있으신가요?</div>'
        ...
        bs +='<tr>' + '<td>' + value[0] + '</td><td>' + value[1] + '</td><td>' + value[2] + '</td><td>' + value[3] + '</td></tr>'
        });
          botui.message
            .bot({
              delay: 2000,
              loading: true,
              type: 'html',
              content: '<div class="answer-table">' + ' <u>수거업체</u> 에요!<br><br>' //+ res.value
          ...
          botui.message
            .bot({
              ...
              loading: true,
              type: 'html',
              content: '<div class="answer-table2"> 폐기물 <u>처리방법</u> 이에요!<br><br>' //+ res.value
              + '<table>\n' +
                  '      <thead>\n' +
                  '        <tr>\n' +
